...
class RotaryEmbedding(nn.Module):
    def __init__(self, dim: int, max_position_embeddings: int, base: int, device = None):
        super().__init__()
        #RoPE
...
class Attention(nn.Module):
    ...
    def forward(self, cos_sin: CosSin, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # hidden_states: [bs, seq_len, num_heads, hidden_dim]
        ...
        #flash attention
        query, key, value = map(lambda t: einops.rearrange(t,'B S H D -> B H S D'), (query, key, value)) # needed for scaled dot product attention
